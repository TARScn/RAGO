...
class PointPillarBaselineMultiscaleRsuSparse(nn.Module):
    """
    RSU-aware multiscale PointPillar with distance and ego sparsity guidance.

    The blend weight becomes large only when:
    1. the location is relatively far from ego; and
    2. the ego-local feature is weak / sparse.
    """
    def __init__(self, args):
        ...
        rsu_cfg = args['rsu_sparse_guidance']
        self.rsu_index = rsu_cfg.get('rsu_index', -1)
        self.level = rsu_cfg.get('level', self.level_num - 1)
        ...
        self.max_alpha = float(rsu_cfg.get('max_alpha', 0.8))
        self.sparsity_temperature = float(
            rsu_cfg.get('sparsity_temperature', 8.0)
        )
        self.sparsity_threshold = float(
            rsu_cfg.get('sparsity_threshold', 0.08)
        )
    ...
    def _build_sparse_prior(self, ego_feature):
        feature_energy = ego_feature.abs().mean(dim=1, keepdim=True)
        sparse_prior = torch.sigmoid(
            (self.sparsity_threshold - feature_energy) *
            self.sparsity_temperature
        )
        return sparse_prior

    def _blend_rsu_feature(self, feature_list, record_len, affine_matrix):
        level_feature = feature_list[self.level]
        split_feature = regroup(level_feature, record_len)
        batch_size = len(split_feature)

        blended_list = []
        level_stride = 2 ** (self.level + 1)
        ...
        for b in range(batch_size):
            ...
            ego_feature = node_features[0:1]
            distance_prior = self._build_distance_prior(
                fused_feature, level_stride
            )
            sparse_prior = self._build_sparse_prior(ego_feature)

            alpha = distance_prior * sparse_prior
            ...
            alpha = (scale * alpha + bias).clamp(0.0, 1.0)

            blended = fused_feature + alpha * (rsu_in_ego - fused_feature)
            blended_list.append(blended)

        feature_list[self.level] = torch.cat(blended_list, dim=0)
        return feature_list
