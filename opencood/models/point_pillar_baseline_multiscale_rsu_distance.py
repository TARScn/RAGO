...
class PointPillarBaselineMultiscaleRsuDistance(nn.Module):
    """
    A lightweight RSU-aware extension of the multiscale PointPillar baseline.

    After normal multiscale fusion, we blend in the RSU feature with a
    distance-aware spatial prior so far-range regions can rely more on RSU's
    larger field of view while near-range regions keep the standard fusion
    result.
    """
    def __init__(self, args):
        ...
        self.learnable_bias = nn.Parameter(
            torch.tensor(float(rsu_cfg.get('bias_init', 0.0)))
        )
        self.alpha_scale = nn.Parameter(
            torch.tensor(float(rsu_cfg.get('alpha_scale_init', 1.0)))
        )
